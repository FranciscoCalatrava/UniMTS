...
class PAMAP():
    ...
    def preprocessing(self):
        ...
        for a in training_cleaned_aux.keys():
            training_cleaned_aux[a]['timestamp'] = training_cleaned_aux[a]['timestamp'] - training_cleaned_aux[a][
                'timestamp'].min()
            ...
            total_missing_timestamps[a] = missing_timestamps[
                a]  # np.union1d(missing_timestamps[a], duplicated_timestamps[a])

            ## We insert the missing indexes
            expected_timestamps = np.arange(training_cleaned_aux[a].index.min(),
                                            training_cleaned_aux[a].index.max() + self.period, self.period)
        ...
        for b in validation_cleaned_aux.keys():
            validation_cleaned_aux[b]['timestamp'] = validation_cleaned_aux[b]['timestamp'] - validation_cleaned_aux[b][
                'timestamp'].min()
            ...
            ## We insert the missing indexes
            expected_timestamps = np.arange(validation_cleaned_aux[b].index.min(),
                                            validation_cleaned_aux[b].index.max() + self.period, self.period)
        ...
        for c in test_cleaned_aux.keys():
            ...
            # We have the timestamps that are missing.
            total_missing_timestamps[a] = missing_timestamps[
                c]  # np.union1d(missing_timestamps[a], duplicated_timestamps[a])

            ## We insert the missing indexes
            expected_timestamps = np.arange(test_cleaned_aux[c].index.min(),
                                            test_cleaned_aux[c].index.max() + self.period, self.period)
    ...
    def prepare_dataset(self):
        ...
        for a in self.training_normalized_segmented.keys():
            for b in self.training_normalized_segmented[a]:
                if (b.iloc[0, 1] == 1 or b.iloc[0, 1] == 2 or b.iloc[0, 1] == 3 or b.iloc[0, 1] == 4 or b.iloc[
                    0, 1] == 5 or b.iloc[0, 1] == 6 or b.iloc[0, 1] == 7 or b.iloc[0, 1] == 12 or b.iloc[0, 1] == 13 or
                        b.iloc[0, 1] == 16 or b.iloc[0, 1] == 17 or b.iloc[0, 1] == 24):
                    training.append(
                        (np.transpose(b.iloc[:, signals].to_numpy()), new_labels[int(b.iloc[0, 1])], int(a)))
    ...
    def dataset_per_sensor_and_participant(self):
        ...
        signal = np.array([4, 5, 6, 10, 11, 12,21, 22, 23, 27, 28, 29,38, 39, 40, 44, 45, 46])
        ...
        for a in self.training_normalized_segmented.keys():
            for b in self.training_normalized_segmented[a]:
                if (b.iloc[0, 1] == 1 or b.iloc[0, 1] == 2 or b.iloc[0, 1] == 3 or b.iloc[0, 1] == 4 or b.iloc[
                    0, 1] == 5 or b.iloc[0, 1] == 6 or b.iloc[0, 1] == 7 or b.iloc[0, 1] == 12 or b.iloc[0, 1] == 13 or
                        b.iloc[0, 1] == 16 or b.iloc[0, 1] == 17 or b.iloc[0, 1] == 24):
                    training.append((np.transpose(b.iloc[:, signal].to_numpy()), new_labels[int(b.iloc[0, 1])],
                                     int(a), int(0)))
                    training.append((np.transpose(b.iloc[:, signal].to_numpy()), new_labels[int(b.iloc[0, 1])],
                                     int(a), int(1)))
                    training.append((np.transpose(b.iloc[:, signal].to_numpy()), new_labels[int(b.iloc[0, 1])],
        ...
        for a in self.validation_normalized_segmented.keys():
            for b in self.validation_normalized_segmented[a]:
                if (b.iloc[0, 1] == 1 or b.iloc[0, 1] == 2 or b.iloc[0, 1] == 3 or b.iloc[0, 1] == 4 or b.iloc[
                    0, 1] == 5 or b.iloc[0, 1] == 6 or b.iloc[0, 1] == 7 or b.iloc[0, 1] == 12 or b.iloc[0, 1] == 13 or
                        b.iloc[0, 1] == 16 or b.iloc[0, 1] == 17 or b.iloc[0, 1] == 24):
                    validation.append((np.transpose(b.iloc[:, hand_signals].to_numpy()), new_labels[int(b.iloc[0, 1])],
                                       int(a), int(0)))
                    validation.append((np.transpose(b.iloc[:, chest_signal].to_numpy()), new_labels[int(b.iloc[0, 1])],
                                       int(a), int(1)))
                    validation.append((np.transpose(b.iloc[:, ankle_signals].to_numpy()), new_labels[int(b.iloc[0, 1])],
                                       int(a), int(2)))
        for a in self.test_normalized_segmented.keys():
            for b in self.test_normalized_segmented[a]:
                if (b.iloc[0, 1] == 1 or b.iloc[0, 1] == 2 or b.iloc[0, 1] == 3 or b.iloc[0, 1] == 4 or b.iloc[
                    0, 1] == 5 or b.iloc[0, 1] == 6 or b.iloc[0, 1] == 7 or b.iloc[0, 1] == 12 or b.iloc[0, 1] == 13 or
                        b.iloc[0, 1] == 16 or b.iloc[0, 1] == 17 or b.iloc[0, 1] == 24):
                    testing.append((np.transpose(b.iloc[:, hand_signals].to_numpy()), new_labels[int(b.iloc[0, 1])],
                                    int(a), int(0)))
                    testing.append((np.transpose(b.iloc[:, chest_signal].to_numpy()), new_labels[int(b.iloc[0, 1])],
                                    int(a), int(1)))
                    testing.append((np.transpose(b.iloc[:, ankle_signals].to_numpy()), new_labels[int(b.iloc[0, 1])],
                                    int(a), int(2)))
